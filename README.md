...
HapExecInsert calls HapExecForeignKeyCheckTriggers and HapBuildHiddenAttr before heap_insert. HapExecForeignKeyCheckTriggers performs foreign key checks on parent tables, calling HAP_RI_FKey_check_before_ins to retrieve the parent's hidden attribute. HapDeconstructParentHiddenAttr then maps these values to the child tuple. After all parent tuples are checked, HapBuildHiddenAttr populates the child tuple's hidden attribute using the mapping table.
