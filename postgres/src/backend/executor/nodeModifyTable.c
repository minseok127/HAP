...
static TupleTableSlot *mergeGetUpdateNewTuple(ResultRelInfo *relinfo,
											  ...
											  MergeActionState *relaction);
#ifdef HAP
static TupleTableSlot *HapExecModifyTable(PlanState *pstate);
#endif /* HAP */
 ...
 */
#ifdef HAP
HAP_HOOK1(TupleTableSlot *, ExecModifyTable,
		  PlanState *, pstate)
#else /* !HAP */
static TupleTableSlot *
					 ...
					 List *child_desc_list, List *encoding_value_list)
{
	...
	forboth(lc, child_desc_list, lc2, encoding_value_list)
		/* Apply it */
		HapApplyEncodingValueToBytea(lfirst(lc), lfirst(lc2), (bytea *) off);
}
...
HapBuildHiddenAttr(HapState *hapstate, TupleTableSlot *slot)
{
	...
	forboth(lc, hap_insert_state->child_desc_map,
		...
		HapInheritHiddenAttr(off, lfirst(lc), lfirst(lc2));
}
					 ...
					 TupleTableSlot *planSlot)
{
	 ...
	 * byte array.
	 */
	hidden_attribute = palloc0(hap_insert_state->nbyte + VARHDRSZ);
...
/*
 * Condition to hook the ExecModifyTable().
 */
HAP_DEFINE_HOOK_COND1(TupleTableSlot *, ExecModifyTable,
					  PlanState *, pstate)
{
	ModifyTableState *node = castNode(ModifyTableState, pstate);
...
/*
 * Hook on ExecModifyTable().
 */
HAP_DEFINE_HOOK_BODY1(TupleTableSlot *, ExecModifyTable,
					  PlanState *, pstate)
{
	ModifyTableState *node = castNode(ModifyTableState, pstate);
	ModifyTableContext context;
