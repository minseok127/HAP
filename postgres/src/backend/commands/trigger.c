...
HapInitHiddenAttrDescState(HapInsertState *state,
						   Oid parent_relid, Oid child_relid)
{
	int16 desccount = HapRelidGetHiddenAttrDescCount(parent_relid);
	List *encoding_value_list = NIL;
	List *parent_desc_list = NIL;
	List *child_desc_list = NIL;

	/* Add new parent */
	state->parent_relid_list = lappend_oid(state->parent_relid_list,
										   parent_relid);

	/* Get the list of parent and child hidden attribute descriptor lists */
	for (int parent_descid = 0; parent_descid < desccount; parent_descid++)
	{
		...
		child_descid = HapGetChildHiddenAttrDescid(child_relid, parent_relid,
												   parent_descid);
