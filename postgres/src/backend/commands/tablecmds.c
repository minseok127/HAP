...
/*
 * HapExtractFkeyConst
 *		Extract foreign key constraints from ColumnDef.
 *
 * CreateStmt has primary elements as a list named tableElts. If the element is
 * a foreign key, add it to the list.
 */
static List *
HapExtractFkeyConst(Node *element)
{
	...
	if (nodeTag(element) == T_ColumnDef)
	{
		...
		/* Each column definition can has constraint */
		foreach(clist, column->constraints)
		{
			constraint = lfirst_node(Constraint, clist);
			if (constraint->contype == CONSTR_FOREIGN)
				ret = lappend(ret, constraint);
		}
	}
	else if (nodeTag(element) == T_Constraint)
	{
		/* Explicit definition of constraint */
 ...
 *		Gather hidden attribute information about the tables referenced by the
 *		new table.
 *
 * CreateStmt contains the tables referenced by the new table. Using it,
 * calculate the total bit size of the hidden attribute and the count of the
 * hidden attribute.
 */
static void
HapGetFKeyTableHiddenAttrInfo(CreateStmt *stmt, int *bitsize, int *count)
{
	...
	*count = 0;
	...
	foreach(lc, stmt->tableElts)
	{
		...
		List *fkeyconstraints;
		ListCell *fkeyCell;

		fkeyconstraints = HapExtractFkeyConst(element);
		foreach(fkeyCell, fkeyconstraints)
		{
			Constraint *fkeyconst = (Constraint *) lfirst(fkeyCell);
			char  *refrelname;

			Assert(fkeyconst->contype == CONSTR_FOREIGN);

			refrelname = fkeyconst->pktable->relname;
			*bitsize += HapRelnameGetHiddenAttrBitsize(refrelname, namespaceId);
			*count += HapRelnameGetHiddenAttrDescCount(refrelname, namespaceId);
		}
	}
}
...
HapAddHiddenAttr(CreateStmt *stmt)
{
	...
	hiddenAttr->raw_default = raw_default;
...
/*
 * HapGetPartitionedId
 *		Get the PARTITIONED table's oid using CreateStmt
 *
 * From Creatstmt->inhRelations, get the oid of PARTITIONED table.
 *
 * Raise error when there is no PARTITIOEND table. So it must be called for
 * PARTITIONs.
 */
static Oid
HapGetPartitionedId(CreateStmt *stmt)
{
	Oid namespaceId;
	RangeVar *rv;

	Assert(stmt->partbound != NULL);
	Assert(list_length(stmt->inhRelations) == 1);

	rv = (RangeVar *) lfirst(list_head(stmt->inhRelations));
	namespaceId = RangeVarGetCreationNamespace(stmt->relation);
	Assert(namespaceId != InvalidOid);

	return get_relname_relid(rv->relname, namespaceId);
}
...
HAP_DEFINE_HOOK_BODY5(ObjectAddress, DefineRelation,
					  CreateStmt *, stmt, char, relkind, Oid, ownerId,
				  	  ObjectAddress *, typaddress, const char *, queryString)
{
	bool partitioned = stmt->partspec != NULL;
	int hiddenAttrBitsize= 0;
	int hiddenAttrDescCount = 0;
	ObjectAddress address;
	char partitionType;
	Relation rel;

	/*
	 * Before creating a tuple descriptor from the relation schema,
	 * we have to add a hidden attribute as a new column.
	 *
	 * Note that in the case of PARTITION, we don't have to add hidden
	 * attribute because it is already added in PARTITIONED table and the
	 * PARTITION will use that schema as is.
	 */
	HapGetFKeyTableHiddenAttrInfo(stmt,
								  &hiddenAttrBitsize, &hiddenAttrDescCount);
	if (!stmt->partbound)
	...
	HapInsertRelTuple(rel, hiddenAttrBitsize, hiddenAttrDescCount);
...
/*
 * HapDefineInternalRel
 *		Create table that is defined by HAP internally.
 *
 * Tables needed by the internal operation of HAP are defined, not by user
 * requests, such as hidden partitions or partition maps.
 */
static Oid
HapDefineInternalRel(Relation parentrel, char *relname, List *tableElts)
{
	Oid			namespaceId;
	Oid			relationId;
	Oid			tablespaceId;
...
/*
 * HapDefinePart
 *		Define hidden partition.
 *
 * Define partition using hidden attributes as partition keys. The attributes
 * of this partition follow those of the parent table.
 */
static Oid
HapDefinePart(Relation parentrel, int id)
{
	List *tableElts, *old_constraints;
	char relname[NAMEDATALEN];

	/* Set name automatically */
	sprintf(relname, "_hap_hidden_partition_%s_%d",
					 RelationGetRelationName(parentrel), id);

	/* Inherit column definitions from parent table */
	tableElts = MergeAttributes(NULL,
								lappend_oid(NIL, RelationGetRelid(parentrel)),
								RELPERSISTENCE_PERMANENT,
								false, &old_constraints);

	return HapDefineInternalRel(parentrel, relname, tableElts);
}
...
{
	ColumnDef *hidden_partition_key, *hidden_partition_oid;
	...
	sprintf(colname2, "_hap_hidden_partition_oid");

	hidden_partition_key = makeColumnDef(colname1, INT4ARRAYOID, -1, 0);
	hidden_partition_oid = makeColumnDef(colname2, OIDOID, -1, 0);

	tableElts = lappend(tableElts, hidden_partition_key);
	tableElts = lappend(tableElts, hidden_partition_oid);

	stmt->partmap_id = HapDefineInternalRel(parentrel, relname, tableElts);	
...
/*
 * HapUpdateCatalogPartMapOids
 *		Update pg_class to record partmap and index oids.
 *
 * Record the partition map's oid and index oids to th pg_class entry.
 *
 * Through this, information about the partition map can be obtained at the smae
 * time as obtatining the relation descriptor from INSERT which requires minimum
 * I/O.
 */
static void
HapUpdateCatalogPartMapOids(HapPartCreateStmt *stmt)
{
	...
	rd_rel->relhappartmapid = stmt->partmap_id;
	rd_rel->relhappartmaphashid = stmt->hash_idx_id;
	rd_rel->relhappartmapginid = stmt->gin_idx_id;
	CatalogTupleUpdate(pg_class, &tuple->t_self, tuple);
...
{
	/* Update pg_class */
	HapUpdateCatalogPartMapOids(stmt);
					  ...
					  LOCKMODE, lockmode)
{
	...
	{
		Oid partid = HapDefinePart(rel, i);
		...
		for (;;)
		{
			...
			HapInsertPartKeyToPartMap(&stmt, stmt.partmap_id, keys, partid);
