...
#ifdef HAP_REDUCTION
/*
 * Change pointer value in list.
 */
List *
list_change_element(List *list, const void *old_ptr, void *new_ptr)
{
	int index;
	ListCell *cell;
	...
	foreach(cell, list)
	{
		const void *element = (void *) lfirst(cell);

		if (element == old_ptr)
		{
			index = list_cell_number(list, cell);
			
			list = list_delete_nth_cell(list, index);
			list = list_insert_nth(list, index, new_ptr);
			break;
		}
	}
