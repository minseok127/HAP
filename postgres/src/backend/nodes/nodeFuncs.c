...
#ifdef HAP_REDUCTION
bool
hap_expression_tree_walker(Node *node, void *context)
{
	ListCell   *temp;
	HapSubstitutionContext *hapContext = (HapSubstitutionContext *) context;
	HapSubstitutionInfo *info = hapContext->info;
	HapSubstitutionCache *cache = hapContext->cache;
	...
	if (IsA(node, Var))
	{
		...
		Assert(hapContext->parent_node != NULL);

		if (info->origin_relid == var->varno && 
						info->origin_attno == var->varattno)
		{
			Node *parent_node = hapContext->parent_node;
			FuncExpr *expr = MakeFuncExprForSubstitution(info, cache);

			switch (nodeTag(parent_node))
			{
				case T_TargetEntry:
					((TargetEntry *) parent_node)->expr = (Expr *) expr;
					//fprintf(stderr, "parent node: T_TargetEntry\n");
					break;
				case T_List:
					list_change_element((List *) parent_node, 
										(void *) node, (void *) expr); 
					//fprintf(stderr, "parent node: T_List\n");
					break;
				case T_FuncExpr:
					//fprintf(stderr, "parent node: T_FuncExpr\n");
					break;
				default:	
					elog(WARNING, "parent node is not expected node. %d, T_TargetEntry : %d", 
												nodeTag(hapContext->parent_node), T_TargetEntry);
			}
		}
	...
	switch (nodeTag(node))
	{
		...
		case T_CTESearchClause:
			...
			break;
		case T_WithCheckOption:
			return hap_expression_tree_walker(((WithCheckOption *) node)->qual, 
																		context);
		case T_Aggref:
			{
				Aggref	   *expr = (Aggref *) node;
				...
				/* recurse directly on List */
				if (hap_expression_tree_walker((Node *) expr->aggdirectargs,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) expr->args,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) expr->aggorder,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) expr->aggdistinct,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) expr->aggfilter, context))
					return true;
			}
			break;
		case T_GroupingFunc:
			{
				...
				if (hap_expression_tree_walker((Node *) grouping->args,
										   context))
					return true;
			}
			break;
		...
		case T_SubscriptingRef:
			{
				...
				/* recurse directly for upper/lower container index lists */
				if (hap_expression_tree_walker((Node *) sbsref->refupperindexpr,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) sbsref->reflowerindexpr,
										   context))
					return true;
				/* walker must see the refexpr and refassgnexpr, however */
				if (hap_expression_tree_walker((Node *) sbsref->refexpr, 
											context))
					return true;

				if (hap_expression_tree_walker((Node *) sbsref->refassgnexpr, 
											context))
					return true;
			}
			break;
		case T_FuncExpr:
			...
			break;
		case T_NamedArgExpr:
			return hap_expression_tree_walker((Node *) 
										((NamedArgExpr *) node)->arg, context);
		case T_OpExpr:
		case T_DistinctExpr:	/* struct-equivalent to OpExpr */
		case T_NullIfExpr:		/* struct-equivalent to OpExpr */
			{
		...
		case T_SubLink:
			{
				...
				if (hap_expression_tree_walker((Node *) sublink->testexpr, 
											context))
					return true;
				 ...
				 * can recurse into the sub-query if it wants to.
				 */
				return hap_expression_tree_walker((Node *) sublink->subselect, 
											context);
			}
			break;
		case T_SubPlan:
			{
				...
				/* recurse into the testexpr, but not into the Plan */
				if (hap_expression_tree_walker((Node *) subplan->testexpr, 
											context))
					return true;
				/* also examine args list */
				if (hap_expression_tree_walker((Node *) subplan->args,
										   context))
					return true;
			}
			break;
		case T_AlternativeSubPlan:
			return hap_expression_tree_walker((Node *) 
							((AlternativeSubPlan *) node)->subplans, context);
		case T_FieldSelect:
			return hap_expression_tree_walker((Node *) 
									((FieldSelect *) node)->arg, context);
		case T_FieldStore:
			{
				FieldStore *fstore = (FieldStore *) node;

				if (hap_expression_tree_walker((Node *) fstore->arg, context))
					return true;
				if (hap_expression_tree_walker((Node *) fstore->newvals, 
										context))
					return true;
			}
			break;
		case T_RelabelType:
			return hap_expression_tree_walker((Node *) 
										((RelabelType *) node)->arg, context);
		case T_CoerceViaIO:
			return hap_expression_tree_walker((Node *) 
										((CoerceViaIO *) node)->arg, context);
		case T_ArrayCoerceExpr:
			{
				ArrayCoerceExpr *acoerce = (ArrayCoerceExpr *) node;

				if (hap_expression_tree_walker((Node *) acoerce->arg, context))
					return true;
				if (hap_expression_tree_walker((Node *) acoerce->elemexpr, 
										context))
					return true;
			}
			break;
		case T_ConvertRowtypeExpr:
			return hap_expression_tree_walker((Node *) 
									((ConvertRowtypeExpr *) node)->arg, context);
		case T_CollateExpr:
			return hap_expression_tree_walker((Node *) 
									((CollateExpr *) node)->arg, context);
		case T_CaseExpr:
			{
				CaseExpr   *caseexpr = (CaseExpr *) node;

				if (hap_expression_tree_walker((Node *) caseexpr->arg, context))
					return true;
				/* we assume walker doesn't care about CaseWhens, either */
				foreach(temp, caseexpr->args)
				{
					...
					if (hap_expression_tree_walker((Node *) when->expr, context))
						return true;
					if (hap_expression_tree_walker((Node *) when->result, 
											context))
						return true;
				}
				if (hap_expression_tree_walker((Node *) caseexpr->defresult, 
											context))
					return true;
			}
			break;
		...
		case T_RowCompareExpr:
			{
				...
				if (hap_expression_tree_walker((Node *) rcexpr->largs, context))
					return true;
				if (hap_expression_tree_walker((Node *) rcexpr->rargs, context))
					return true;
			}
			break;
		case T_CoalesceExpr:
			return hap_expression_tree_walker((Node *) 
										((CoalesceExpr *) node)->args, context);
		case T_MinMaxExpr:
			return hap_expression_tree_walker((Node *) 
		...
		case T_XmlExpr:
			{
				...
				if (hap_expression_tree_walker((Node *) xexpr->named_args, 
										context))
					return true;
				/* we assume walker doesn't care about arg_names */
				if (hap_expression_tree_walker((Node *) xexpr->args, context))
					return true;
			}
			break;
		case T_NullTest:
			return hap_expression_tree_walker((Node *) 
											((NullTest *) node)->arg, context);
		case T_BooleanTest:
			return hap_expression_tree_walker((Node *) 
										((BooleanTest *) node)->arg, context);
		case T_CoerceToDomain:
			return hap_expression_tree_walker((Node *) 
										((CoerceToDomain *) node)->arg, context);
		case T_TargetEntry:
			return hap_expression_tree_walker((Node *) 
		...
		case T_WindowClause:
			{
				...
				if (hap_expression_tree_walker((Node *) wc->partitionClause, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) wc->orderClause, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) wc->startOffset, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) wc->endOffset, context))
					return true;
			}
			break;
		case T_CTECycleClause:
			{
				...
				if (hap_expression_tree_walker((Node *) cc->cycle_mark_value, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) cc->cycle_mark_default, 
												context))
					return true;
			}
			break;
		case T_CommonTableExpr:
			{
				 ...
				 * recurse into the sub-query if it wants to.
				 */
				if (hap_expression_tree_walker((Node *) cte->ctequery, context))
					return true;

				if (hap_expression_tree_walker((Node *) cte->search_clause, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) cte->cycle_clause, 
												context))
					return true;
			}
			break;
		case T_PartitionBoundSpec:
			{
				...
				if (hap_expression_tree_walker((Node *) pbs->listdatums, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) pbs->lowerdatums, 
												context))
					return true;
				if (hap_expression_tree_walker((Node *) pbs->upperdatums, 
												context))
					return true;
			}
			break;
		case T_PartitionRangeDatum:
			{
				...
				if (hap_expression_tree_walker((Node *) prd->value, context))
					return true;
			}
			break;
		case T_List:
			foreach(temp, (List *) node)
			{
				if (hap_expression_tree_walker((Node *) lfirst(temp), context))
					return true;
			}
			break;
		case T_FromExpr:
			{
				...
				if (hap_expression_tree_walker((Node *) from->fromlist, context))
					return true;
				if (hap_expression_tree_walker((Node *) from->quals, context))
					return true;
			}
			break;
		case T_OnConflictExpr:
			{
				...
				if (hap_expression_tree_walker((Node *) 
										onconflict->arbiterElems, context))
					return true;
				if (hap_expression_tree_walker((Node *) 
										onconflict->arbiterWhere, context))
					return true;
				if (hap_expression_tree_walker((Node *) 
										onconflict->onConflictSet, context))
					return true;
				if (hap_expression_tree_walker((Node *) 
										onconflict->onConflictWhere, context))
					return true;
				if (hap_expression_tree_walker((Node *) 
										onconflict->exclRelTlist, context))
					return true;
			}
			break;
		case T_MergeAction:
			{
				...
				if (hap_expression_tree_walker((Node *) action->qual, context))
					return true;
				if (hap_expression_tree_walker((Node *) action->targetList, 
										context))
					return true;
			}
			break;
		case T_PartitionPruneStepOp:
			{
				...
				if (hap_expression_tree_walker((Node *) opstep->exprs, context))
					return true;
			}
			break;
		case T_PartitionPruneStepCombine:
		...
		case T_JoinExpr:
			{
				...
				if (hap_expression_tree_walker((Node *) join->larg, context))
					return true;
				if (hap_expression_tree_walker((Node *) join->rarg, context))
					return true;
				if (hap_expression_tree_walker((Node *) join->quals, context))
					return true;
		...
		case T_SetOperationStmt:
			{
				...
				if (hap_expression_tree_walker((Node *) setop->larg, context))
					return true;
				if (hap_expression_tree_walker((Node *) setop->rarg, context))
					return true;
		...
		case T_IndexClause:
			{
				...
				if (hap_expression_tree_walker((Node *) iclause->rinfo, context))
					return true;
				if (hap_expression_tree_walker((Node *) iclause->indexquals,
										   context))
					return true;
			}
			break;
		case T_PlaceHolderVar:
			return hap_expression_tree_walker((Node *) 
									((PlaceHolderVar *) node)->phexpr, context);
		case T_InferenceElem:
			return hap_expression_tree_walker((Node *) 
									((InferenceElem *) node)->expr, context);
		case T_AppendRelInfo:
			{
				AppendRelInfo *appinfo = (AppendRelInfo *) node;

				if (hap_expression_tree_walker((Node *) appinfo->translated_vars,
										   context))
					return true;
			}
			break;
		case T_PlaceHolderInfo:
			return hap_expression_tree_walker((Node *) 
									((PlaceHolderInfo *) node)->ph_var, context);
		case T_RangeTblFunction:
			return hap_expression_tree_walker((Node *) 
								((RangeTblFunction *) node)->funcexpr, context);
		case T_TableSampleClause:
			{
				TableSampleClause *tsc = (TableSampleClause *) node;

				if (hap_expression_tree_walker((Node *) tsc->args,
										   context))
					return true;
				if (hap_expression_tree_walker((Node *) tsc->repeatable, context))
					return true;
			}
			break;
		case T_TableFunc:
			{
				...
				if (hap_expression_tree_walker((Node *) tf->ns_uris, context))
					return true;
				if (hap_expression_tree_walker((Node *) tf->docexpr, context))
					return true;
				if (hap_expression_tree_walker((Node *) tf->rowexpr, context))
					return true;
				if (hap_expression_tree_walker((Node *) tf->colexprs, context))
					return true;
				if (hap_expression_tree_walker((Node *) tf->coldefexprs, context))
					return true;
			}
			break;
		default:
