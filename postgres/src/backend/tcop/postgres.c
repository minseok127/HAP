...
#ifdef TEMP_IO
uint64_t agg_temp_io_time_nsec = 0;
uint64_t join_temp_io_time_nsec = 0;
uint64_t scan_time_nsec = 0;
uint64_t total_nsec = 0;
uint64_t qual_time_nsec = 0;
uint64_t project_time_nsec = 0;
extern bool isOLAP;
#endif /* TEMP_IO */
...
exec_simple_query(const char *query_string)
{
...
#ifdef TEMP_IO
	agg_temp_io_time_nsec = 0;
	join_temp_io_time_nsec = 0;
	scan_time_nsec = 0;
	qual_time_nsec = 0;
	project_time_nsec = 0;
	total_nsec = 0;
	...
	if (isOLAP)
	{
		...
		total_nsec = (end.tv_sec - start.tv_sec) * 1e9 + (end.tv_nsec - start.tv_nsec);

		fprintf(stderr, "[TEMP_IO] total_nsec: %llu, agg_temp_io_time_nsec: %llu, join_temp_io_time_nsec: %llu, scan_time_nsec: %llu, project_time_nsec: %llu, qual_time_nsec: %llu\n", 
				total_nsec, agg_temp_io_time_nsec, join_temp_io_time_nsec,
				scan_time_nsec, project_time_nsec, qual_time_nsec);
